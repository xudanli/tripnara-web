...
import type { UserTrustProfile } from '@/types/identity-governance';

interface TrustProfilePublicViewProps {
  profile: UserTrustProfile | undefined;
  isLoading?: boolean;
  isError?: boolean;
}

export function TrustProfilePublicView({
  profile,
  isLoading,
  isError,
}: TrustProfilePublicViewProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">
            {profile.displayName ?? '旅行者'}
          </CardTitle>
          <CardDescription>公开信任档案 · 无综合信用分</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {profile.professional?.isVerifiedProfessional && (
            <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-4 py-3">
              ...
              {profile.professional.destinations?.length ? (
                <p className="mt-2 text-xs text-muted-foreground">
                  目的地：{profile.professional.destinations.join(' · ')}
                  {profile.professional.yearsOfExperience != null &&
                    ` · ${profile.professional.yearsOfExperience} 年经验`}
                </p>
              ) : null}
            </div>
          )}
      ...
      {profile.endorsements.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">机构背书</CardTitle>
      ...
      <p className="text-center text-xs text-muted-foreground">
        信任档案由验证、资质、背书与声誉事实组成，不代表社交信用评分。
