...
export default function AutoView({ trip, onItemClick }: AutoViewProps) {
  const { t } = useTranslation();
  const [activeTab, setActiveTab] = useState<'overview' | 'abu' | 'dre' | 'neptune'>('overview');

  // 计算综合指标（可以从后端获取）
  const overallMetrics = {
    safetyScore: 75, // 安全评分 0-100
    rhythmScore: 80, // 节奏评分 0-100
    readinessScore: 85, // 准备度评分 0-100
    criticalIssues: 2, // 关键问题数
    warnings: 3, // 警告数
    suggestions: 5, // 建议数
  };
  ...
  const safetyBadge = getSafetyBadge(overallMetrics.safetyScore);
    ...
    <div className="space-y-6">
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-sm font-semibold ${getSafetyColor(overallMetrics.safetyScore)}`}>
                      {overallMetrics.safetyScore}/100
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">关键问题</span>
                    <Badge variant={overallMetrics.criticalIssues > 0 ? 'destructive' : 'secondary'} className="text-xs">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-orange-600">
                      {overallMetrics.rhythmScore}/100
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">警告</span>
                    <Badge variant={overallMetrics.warnings > 0 ? 'default' : 'secondary'} className="text-xs">
                      {overallMetrics.warnings}
                    </Badge>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-green-600">
                      {overallMetrics.readinessScore}/100
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">建议</span>
                    <Badge variant="outline" className="text-xs">
                      {overallMetrics.suggestions}
                    </Badge>
                  </div>
                </div>
              </CardContent>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">疲劳指数</span>
                    </div>
        ...
        <TabsContent value="abu">
          <AbuView trip={trip} onItemClick={onItemClick} />
        </TabsContent>

        {/* Dr.Dre 详细视图 */}
        <TabsContent value="dre">
          <DrDreView trip={trip} onItemClick={onItemClick} />
        </TabsContent>

        {/* Neptune 详细视图 */}
        <TabsContent value="neptune">
          <NeptuneView trip={trip} onItemClick={onItemClick} />
        </TabsContent>
      </Tabs>
    </div>
  );
