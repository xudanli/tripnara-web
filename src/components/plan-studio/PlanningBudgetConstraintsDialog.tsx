...
import { PLANNING_CONSTRAINT_EDIT_META } from '@/lib/planning-constraint-edit-meta';
...
export function PlanningBudgetConstraintsDialog({
  ...
  const meta = PLANNING_CONSTRAINT_EDIT_META.budget;
  ...
  return (
    <PlanningConstraintDialogShell
      ...
      tripId={tripId}
      saving={saving}
      saveDisabled={!canSave || (loading && !profile)}
      onSave={handleSave}
      footerExtra={
          ...
          {draftTotal >= 100 ? (
            <section className="rounded-lg border border-border/70 p-3.5 space-y-3">
              ...
              <ul className="space-y-1.5">
                {STRUCTURE_CATEGORY_META.filter((m) => m.key !== 'other').map((metaItem) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <span className="text-[11px] text-muted-foreground">{metaItem.labelZh}</span>
                        <Input
                          type="number"
                          min={0}
                          className="mt-0.5 h-7 text-xs tabular-nums"
                          value={draftAlloc[metaItem.key] || ''}
                          onChange={(e) =>
                            setDraftAlloc((prev) => ({
                              ...prev,
                              [metaItem.key]: Math.max(0, Number(e.target.value) || 0),
                            }))
                          }
                        />
                      </div>
                      <span className="text-[11px] text-muted-foreground tabular-nums text-right">
                        {draftPct[metaItem.key].toFixed(0)}%
                      </span>
                    </li>
                  );
                })}
            ...
            <p className="text-xs text-muted-foreground px-0.5">设置总预算后可分配消费结构</p>
          )}
        </div>
      )}
    </PlanningConstraintDialogShell>
