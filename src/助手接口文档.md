# 智能体助手 API 文档

## 概述

本文档描述两个智能体助手的 API 接口：
- **规划助手 (Planning Assistant)**: 帮用户规划旅行
- **行程助手 (Journey Assistant)**: 陪用户执行旅程

**Base URL**: `/api/agent`

---

## 一、规划助手 API

### 1.1 创建会话

开始一个新的旅行规划对话会话。

```
POST /agent/planning-assistant/sessions
```

#### 请求体

```typescript
interface CreateSessionRequest {
  userId?: string;  // 用户ID（可选）
}
```

#### 响应

```typescript
interface CreateSessionResponse {
  sessionId: string;  // 会话ID
}
```

#### 示例

**请求**:
```bash
curl -X POST http://localhost:3000/api/agent/planning-assistant/sessions \
  -H "Content-Type: application/json" \
  -d '{}'
```

**响应**:
```json
{
  "sessionId": "cf50c461-e727-45d6-a50f-c6fe1dff3840"
}
```

---

### 1.2 对话

向规划助手发送消息，获取智能回复、推荐和行程方案。

```
POST /agent/planning-assistant/chat
```

#### 请求体

```typescript
interface PlanningChatRequest {
  sessionId: string;           // 会话ID（必填）
  message: string;             // 用户消息（必填）
  userId?: string;             // 用户ID
  language?: 'en' | 'zh';      // 语言偏好
  context?: {
    currentLocation?: {        // 当前位置
      lat: number;
      lng: number;
    };
    timezone?: string;         // 时区
  };
}
```

#### 响应

```typescript
interface PlanningChatResponse {
  // 回复消息
  message: string;             // 英文回复
  messageCN: string;           // 中文回复
  
  // 当前对话阶段
  phase: 'INITIAL' | 'EXPLORING' | 'RECOMMENDING' | 'PLANNING' | 'COMPARING' | 'ADJUSTING' | 'CONFIRMING' | 'COMPLETED';
  
  // 引导问题（探索阶段）
  guidingQuestions?: {
    question: string;
    questionCN: string;
    options?: string[];
    optionsCN?: string[];
    type: 'single' | 'multiple' | 'text' | 'date' | 'number';
  }[];
  
  // 目的地推荐（推荐阶段）
  recommendations?: DestinationRecommendation[];
  
  // 方案候选（规划阶段）
  planCandidates?: PlanCandidate[];
  
  // 方案对比（对比阶段）
  comparison?: {
    dimensions: string[];
    candidates: { id: string; name: string; scores: Record<string, number>; }[];
    recommendation: string;
    recommendationCN: string;
  };
  
  // 确认的行程ID（完成阶段）
  confirmedTripId?: string;
  
  // 建议操作
  suggestedActions?: {
    action: string;
    label: string;
    labelCN: string;
  }[];
}

// 目的地推荐
interface DestinationRecommendation {
  id: string;
  countryCode: string;
  name: string;
  nameCN: string;
  description: string;
  descriptionCN: string;
  highlights: string[];
  highlightsCN: string[];
  matchScore: number;           // 匹配分数 0-100
  matchReasons: string[];
  matchReasonsCN: string[];
  estimatedBudget: {
    min: number;
    max: number;
    currency: string;
  };
  bestSeasons: string[];
  imageUrl?: string;
  tags: string[];
}

// 方案候选
interface PlanCandidate {
  id: string;
  name: string;
  nameCN: string;
  description: string;
  descriptionCN: string;
  destination: string;
  duration: number;              // 天数
  highlights: string[];
  estimatedBudget: {
    total: number;
    breakdown: {
      flight: number;
      accommodation: number;
      activities: number;
      food: number;
      other: number;
    };
  };
  pace: 'relaxed' | 'moderate' | 'intensive';
  suitability: {
    score: number;
    reasons: string[];
  };
  warnings?: string[];
}
```

#### 示例

**请求 - 探索阶段**:
```bash
curl -X POST http://localhost:3000/api/agent/planning-assistant/chat \
  -H "Content-Type: application/json" \
  -d '{
    "sessionId": "cf50c461-e727-45d6-a50f-c6fe1dff3840",
    "message": "我想去旅行"
  }'
```

**响应**:
```json
{
  "message": "Great! I'd love to help you plan your trip! 🌟\n\nTo give you the best recommendations, I'd like to know a bit more:\n\n1. **When** are you planning to travel?\n2. **Who** will be traveling?\n3. **What's your budget** range?",
  "messageCN": "太好了！我很乐意帮你规划旅行！🌟\n\n为了给你更好的推荐，我想先了解一下：\n\n1. **什么时候**出发？\n2. **谁一起去**？\n3. **预算大概多少**？",
  "phase": "EXPLORING",
  "guidingQuestions": [
    {
      "question": "When are you planning to travel?",
      "questionCN": "计划什么时候出发？",
      "type": "text"
    },
    {
      "question": "Who will be traveling?",
      "questionCN": "谁一起去？",
      "options": ["Solo", "Couple", "Family", "Friends"],
      "optionsCN": ["独自出行", "情侣", "家庭", "朋友"],
      "type": "single"
    }
  ]
}
```

**请求 - 生成方案**:
```bash
curl -X POST http://localhost:3000/api/agent/planning-assistant/chat \
  -H "Content-Type: application/json" \
  -d '{
    "sessionId": "cf50c461-e727-45d6-a50f-c6fe1dff3840",
    "message": "我想去冰岛玩10天，预算3万"
  }'
```

**响应**:
```json
{
  "message": "I've created 3 itinerary options for 冰岛...",
  "messageCN": "我为你的冰岛之旅创建了 3 个方案：\n\n1. **悠闲探索者** - $4,500\n   10天 | 舒适节奏...\n\n🐻‍❄️ **Abu 说**: 经过安全检查，当前方案在物理现实和合规性方面没有问题。\n🐕 **Dr.Dre 说**: 当前节奏合理，每一天都刚刚好。\n🦦 **Neptune 说**: 当前方案在空间和路线哲学方面没有问题。",
  "phase": "PLANNING",
  "planCandidates": [
    {
      "id": "plan-relaxed",
      "name": "Relaxed Explorer",
      "nameCN": "悠闲探索者",
      "description": "Comfortable pace with time to enjoy",
      "descriptionCN": "舒适节奏，充分享受每个目的地",
      "destination": "冰岛",
      "duration": 10,
      "highlights": ["Scenic views", "Local cuisine", "Cultural sites"],
      "estimatedBudget": {
        "total": 4500,
        "breakdown": {
          "flight": 1200,
          "accommodation": 1800,
          "activities": 800,
          "food": 500,
          "other": 200
        }
      },
      "pace": "relaxed",
      "suitability": {
        "score": 92,
        "reasons": ["Matches pace preference", "Within budget"]
      }
    }
  ],
  "suggestedActions": [
    { "action": "view_plan-relaxed", "label": "View Relaxed Explorer", "labelCN": "查看悠闲探索者" }
  ]
}
```

**请求 - 确认方案**:
```bash
curl -X POST http://localhost:3000/api/agent/planning-assistant/chat \
  -H "Content-Type: application/json" \
  -d '{
    "sessionId": "cf50c461-e727-45d6-a50f-c6fe1dff3840",
    "message": "就选精华平衡版，确认"
  }'
```

**响应**:
```json
{
  "message": "🎉 Excellent choice! Your trip has been confirmed!...",
  "messageCN": "🎉 太棒了！你的行程已确认！\n\n**行程编号**: trip-1768584179404\n**目的地**: 冰岛\n**时长**: 10天\n**方案**: 精华平衡版\n\n接下来可以：\n- 查看详细行程安排\n- 开始准备（打包清单、签证信息等）\n- 分享给同行伙伴\n\n祝你旅途愉快！🌟",
  "phase": "COMPLETED",
  "confirmedTripId": "trip-1768584179404",
  "suggestedActions": [
    { "action": "view_itinerary", "label": "View Itinerary", "labelCN": "查看行程" },
    { "action": "start_preparing", "label": "Start Preparing", "labelCN": "开始准备" }
  ]
}
```

---

### 1.3 获取会话状态

获取指定会话的当前状态。

```
GET /agent/planning-assistant/sessions/:sessionId
```

#### 路径参数

| 参数 | 类型 | 说明 |
|------|------|------|
| sessionId | string | 会话ID |

#### 响应

```typescript
interface SessionStateResponse {
  sessionId: string;
  userId?: string;
  phase: string;
  preferences: UserPreferences;
  recommendations?: DestinationRecommendation[];
  selectedDestination?: string;
  planCandidates?: PlanCandidate[];
  selectedPlanId?: string;
  confirmedTripId?: string;
  messageCount: number;
  createdAt: string;
  updatedAt: string;
}

interface UserPreferences {
  travelers?: {
    adults?: number;
    children?: number;
    childrenAges?: number[];
  };
  dateRange?: {
    startDate?: string;
    endDate?: string;
    flexible?: boolean;
    preferredMonths?: number[];
  };
  budget?: {
    total?: number;
    currency?: string;
    level?: 'low' | 'medium' | 'high' | 'luxury';
  };
  destination?: {
    continents?: string[];
    countries?: string[];
    type?: ('beach' | 'city' | 'nature' | 'culture' | 'adventure')[];
  };
  activities?: {
    preferred?: string[];
    avoid?: string[];
    pacePreference?: 'relaxed' | 'moderate' | 'intensive';
  };
}
```

#### 示例

```bash
curl http://localhost:3000/api/agent/planning-assistant/sessions/cf50c461-e727-45d6-a50f-c6fe1dff3840
```

---

### 1.4 快速推荐

无需创建会话，直接获取目的地推荐。

```
GET /agent/planning-assistant/quick-recommend
```

#### 查询参数

| 参数 | 类型 | 说明 |
|------|------|------|
| budget | string | 预算（可选） |
| travelersCount | string | 人数（可选） |
| preferredType | string | 偏好类型（可选） |
| language | 'en' \| 'zh' | 语言（可选，默认 zh） |

#### 示例

```bash
curl "http://localhost:3000/api/agent/planning-assistant/quick-recommend?budget=30000&travelersCount=2&preferredType=nature"
```

---

### 1.5 获取用户偏好摘要 (P1 新功能)

获取系统学习到的用户旅行偏好，用于个性化推荐。

```
GET /agent/planning-assistant/users/:userId/preferences
```

#### 路径参数

| 参数 | 类型 | 说明 |
|------|------|------|
| userId | string | 用户ID |

#### 响应

```typescript
interface UserPreferenceSummaryResponse {
  summary: string;           // 英文摘要
  summaryCN: string;         // 中文摘要
  topPreferences: {
    label: string;           // 标签（英文）
    labelCN: string;         // 标签（中文）
    value: string;           // 值
  }[];
  learnedPreferences?: {     // 学习到的偏好详情
    destination?: { ... };
    budget?: { total: number };
    travelers?: { adults?: number; children?: number };
    activities?: { preferred?: string[] };
  };
}
```

#### 示例

```bash
curl http://localhost:3000/api/agent/planning-assistant/users/user-123/preferences
```

**响应**:
```json
{
  "summary": "2 trips completed, avg budget $4500, prefer 10-day trips",
  "summaryCN": "已完成 2 次旅行，平均预算 $4500，偏好 10 天行程",
  "topPreferences": [
    { "label": "Budget", "labelCN": "预算", "value": "$4500" },
    { "label": "Duration", "labelCN": "时长", "value": "10 days" },
    { "label": "Destination Type", "labelCN": "目的地类型", "value": "nature, adventure" }
  ],
  "learnedPreferences": {
    "budget": { "total": 4500 },
    "destination": { "type": ["nature", "adventure"] }
  }
}
```

---

### 1.6 清除用户偏好 (P1 新功能)

清除系统学习到的用户旅行偏好。

```
POST /agent/planning-assistant/users/:userId/preferences/clear
```

#### 路径参数

| 参数 | 类型 | 说明 |
|------|------|------|
| userId | string | 用户ID |

#### 响应

```json
{ "success": true }
```

#### 示例

```bash
curl -X POST http://localhost:3000/api/agent/planning-assistant/users/user-123/preferences/clear
```

---

## 二、行程助手 API

### 2.1 对话

旅途中与行程助手对话。

```
POST /agent/journey-assistant/chat
```

#### 请求体

```typescript
interface JourneyChatRequest {
  tripId: string;              // 行程ID（必填）
  userId: string;              // 用户ID（必填）
  message: string;             // 用户消息（必填）
  language?: 'en' | 'zh';      // 语言偏好
  context?: {
    currentLocation?: {        // 当前位置
      lat: number;
      lng: number;
    };
    timezone?: string;         // 时区
  };
}
```

#### 响应

```typescript
interface JourneyAssistantResponse {
  message?: string;            // 英文回复
  messageCN?: string;          // 中文回复
  journeyState?: JourneyState; // 行程状态
  reminders?: Reminder[];      // 提醒列表
  event?: TripEvent;           // 事件信息
  options?: EmergencyOption[]; // 应急方案
  adjustmentResult?: AdjustmentResult;  // 调整结果
  searchResults?: SearchResults;        // 搜索结果
  suggestedActions?: SuggestedAction[]; // 建议操作
}

interface JourneyState {
  tripId: string;
  userId: string;
  phase: 'PRE_TRIP' | 'DEPARTURE_DAY' | 'ON_TRIP' | 'RETURN_DAY' | 'POST_TRIP';
  currentDay: number;
  totalDays: number;
  currentDate: string;
  currentLocation?: { lat: number; lng: number; name?: string; };
  todaySchedule: ScheduleItem[];
  upcomingReminders: Reminder[];
  activeEvents: TripEvent[];
  stats: {
    completedActivities: number;
    totalActivities: number;
    spentBudget: number;
    totalBudget: number;
  };
  lastUpdated: string;
}

interface ScheduleItem {
  id: string;
  type: 'flight' | 'hotel' | 'activity' | 'transport' | 'meal' | 'rest';
  title: string;
  titleCN: string;
  startTime: string;
  endTime?: string;
  location?: {
    name: string;
    nameCN: string;
    lat: number;
    lng: number;
    address?: string;
  };
  status: 'upcoming' | 'in_progress' | 'completed' | 'cancelled' | 'modified';
  notes?: string;
  notesCN?: string;
}

interface Reminder {
  id: string;
  type: 'FLIGHT' | 'HOTEL' | 'ACTIVITY' | 'TRANSPORT' | 'WEATHER' | 'SAFETY' | 'DOCUMENT' | 'PACKING' | 'BUDGET';
  title: string;
  titleCN: string;
  message: string;
  messageCN: string;
  priority: 'low' | 'medium' | 'high' | 'urgent';
  scheduledAt: string;
  relatedItemId?: string;
  actionRequired?: boolean;
  actions?: { action: string; label: string; labelCN: string; }[];
}
```

#### 示例

**请求 - 附近搜索**:
```bash
curl -X POST http://localhost:3000/api/agent/journey-assistant/chat \
  -H "Content-Type: application/json" \
  -d '{
    "tripId": "trip-123",
    "userId": "user-456",
    "message": "附近有什么好吃的"
  }'
```

**响应**:
```json
{
  "message": "I found 3 restaurants near you:\n\n1. **Restaurant A** - 200m away ⭐ 4.5\n2. **Restaurant B** - 350m away ⭐ 4.3\n3. **Restaurant C** - 500m away ⭐ 4.7\n\nWould you like me to navigate you to any of these?",
  "messageCN": "我在附近找到了 3 家餐厅：\n\n1. **餐厅A** - 200米 ⭐ 4.5\n2. **餐厅B** - 350米 ⭐ 4.3\n3. **餐厅C** - 500米 ⭐ 4.7\n\n需要我帮你导航到哪一家吗？",
  "searchResults": {
    "type": "restaurants",
    "items": [
      { "name": "Restaurant A", "nameCN": "餐厅A", "distance": "200m", "rating": 4.5 }
    ]
  },
  "suggestedActions": [
    { "action": "navigate_1", "label": "Navigate to Restaurant A", "labelCN": "导航到餐厅A" }
  ]
}
```

---

### 2.2 获取行程状态

获取当前行程的状态。

```
GET /agent/journey-assistant/trips/:tripId/status
```

#### 路径参数

| 参数 | 类型 | 说明 |
|------|------|------|
| tripId | string | 行程ID |

#### 示例

```bash
curl http://localhost:3000/api/agent/journey-assistant/trips/trip-123/status
```

**响应**:
```json
{
  "message": "📍 Day 1 of 10\n\nCurrent status: ON TRIP\nToday's activities: 2\nCompleted: 3/15\nBudget used: $1200/$5000",
  "messageCN": "📍 第 1 天 / 共 10 天\n\n当前状态：旅途中\n今日活动：2 项\n已完成：3/15\n已用预算：¥8400/¥35000",
  "journeyState": {
    "tripId": "trip-123",
    "phase": "ON_TRIP",
    "currentDay": 1,
    "totalDays": 10,
    "todaySchedule": [
      {
        "id": "s1",
        "type": "activity",
        "title": "Golden Circle Tour",
        "titleCN": "黄金圈一日游",
        "startTime": "2026-01-16T09:00:00",
        "status": "in_progress"
      }
    ],
    "stats": {
      "completedActivities": 3,
      "totalActivities": 15,
      "spentBudget": 1200,
      "totalBudget": 5000
    }
  },
  "suggestedActions": [
    { "action": "view_schedule", "label": "View today's schedule", "labelCN": "查看今日行程" }
  ]
}
```

---

### 2.3 获取提醒列表

获取当前行程的所有待办提醒。

```
GET /agent/journey-assistant/trips/:tripId/reminders
```

#### 示例

```bash
curl http://localhost:3000/api/agent/journey-assistant/trips/trip-123/reminders
```

---

### 2.4 处理突发事件

处理航班延误、景点关闭等突发事件。

```
POST /agent/journey-assistant/events/handle
```

#### 请求体

```typescript
interface HandleEventRequest {
  tripId: string;              // 行程ID（必填）
  userId: string;              // 用户ID（必填）
  eventId: string;             // 事件ID（必填）
  selectedOptionId?: string;   // 选择的方案ID（可选）
  language?: 'en' | 'zh';
  context?: { ... };
}
```

#### 示例

```bash
curl -X POST http://localhost:3000/api/agent/journey-assistant/events/handle \
  -H "Content-Type: application/json" \
  -d '{
    "tripId": "trip-123",
    "userId": "user-456",
    "eventId": "event-789"
  }'
```

---

### 2.5 调整行程

调整行程安排（改时间、取消活动等）。

```
POST /agent/journey-assistant/schedule/adjust
```

#### 请求体

```typescript
interface AdjustScheduleRequest {
  tripId: string;
  userId: string;
  adjustmentParams: {
    itemId: string;            // 行程项ID
    newTime?: string;          // 新时间
    cancel?: boolean;          // 是否取消
    replace?: {                // 替换内容
      type: string;
      details: any;
    };
  };
  language?: 'en' | 'zh';
}
```

#### 示例

```bash
curl -X POST http://localhost:3000/api/agent/journey-assistant/schedule/adjust \
  -H "Content-Type: application/json" \
  -d '{
    "tripId": "trip-123",
    "userId": "user-456",
    "adjustmentParams": {
      "itemId": "s1",
      "newTime": "2026-01-16T14:00:00"
    }
  }'
```

---

### 2.6 紧急求助

紧急情况下获取帮助。

```
POST /agent/journey-assistant/emergency
```

#### 请求体

```typescript
interface EmergencyRequest {
  tripId: string;
  userId: string;
  language?: 'en' | 'zh';
  context?: {
    currentLocation?: { lat: number; lng: number; };
  };
}
```

#### 响应示例

```json
{
  "message": "🚨 I'm here to help! What's the emergency?",
  "messageCN": "🚨 我来帮你！发生了什么紧急情况？\n\n**快速操作：**\n- 🏥 查找最近医院\n- 👮 联系当地警察\n- 🏛️ 联系大使馆\n- 📞 紧急热线：112",
  "suggestedActions": [
    { "action": "find_hospital", "label": "Find hospital", "labelCN": "找医院" },
    { "action": "call_police", "label": "Call police", "labelCN": "报警" },
    { "action": "contact_embassy", "label": "Contact embassy", "labelCN": "联系大使馆" }
  ]
}
```

---

### 2.7 附近搜索

搜索附近的餐厅、景点等。

```
POST /agent/journey-assistant/nearby
```

#### 请求体

```typescript
interface NearbySearchRequest {
  tripId: string;
  userId: string;
  message: string;             // 搜索内容，如"附近有什么好吃的"
  language?: 'en' | 'zh';
  context?: {
    currentLocation?: { lat: number; lng: number; };
  };
}
```

---

## 三、对话阶段流程

### 规划助手对话流程

```
INITIAL → EXPLORING → RECOMMENDING → PLANNING → COMPARING → ADJUSTING → CONFIRMING → COMPLETED
```

| 阶段 | 说明 | 主要输出 |
|------|------|----------|
| INITIAL | 初始阶段 | 欢迎语 |
| EXPLORING | 探索阶段 | guidingQuestions |
| RECOMMENDING | 推荐阶段 | recommendations |
| PLANNING | 规划阶段 | planCandidates |
| COMPARING | 对比阶段 | comparison |
| ADJUSTING | 调整阶段 | planCandidates (更新) |
| CONFIRMING | 确认阶段 | confirmedTripId |
| COMPLETED | 完成阶段 | suggestedActions |

### 行程助手阶段

| 阶段 | 说明 | 触发时机 |
|------|------|----------|
| PRE_TRIP | 出发前 | 出发前1-7天 |
| DEPARTURE_DAY | 出发当天 | 出发当天 |
| ON_TRIP | 旅途中 | 旅行期间 |
| RETURN_DAY | 返程当天 | 返程当天 |
| POST_TRIP | 旅行结束后 | 返程后 |

---

## 四、错误响应

所有接口在出错时返回统一格式：

```typescript
interface ErrorResponse {
  statusCode: number;
  timestamp: string;
  path: string;
  method: string;
  message: string[];
}
```

### 常见错误码

| 状态码 | 说明 |
|--------|------|
| 400 | 请求参数错误 |
| 401 | 未授权（需要登录） |
| 404 | 会话/行程不存在 |
| 500 | 服务器内部错误 |

---

## 五、前端集成示例

### TypeScript 类型定义

```typescript
// types/assistant.ts

export interface PlanningSession {
  sessionId: string;
}

export interface PlanningChatRequest {
  sessionId: string;
  message: string;
  userId?: string;
  language?: 'en' | 'zh';
}

export interface PlanningChatResponse {
  message: string;
  messageCN: string;
  phase: string;
  guidingQuestions?: GuidingQuestion[];
  recommendations?: DestinationRecommendation[];
  planCandidates?: PlanCandidate[];
  confirmedTripId?: string;
  suggestedActions?: SuggestedAction[];
}
```

### React Hook 示例

```typescript
// hooks/usePlanningAssistant.ts

import { useState, useCallback } from 'react';
import axios from 'axios';

const API_BASE = '/api/agent/planning-assistant';

export function usePlanningAssistant() {
  const [sessionId, setSessionId] = useState<string | null>(null);
  const [messages, setMessages] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);

  // 创建会话
  const createSession = useCallback(async () => {
    const { data } = await axios.post(`${API_BASE}/sessions`);
    setSessionId(data.sessionId);
    return data.sessionId;
  }, []);

  // 发送消息
  const sendMessage = useCallback(async (message: string) => {
    if (!sessionId) {
      await createSession();
    }
    
    setLoading(true);
    try {
      const { data } = await axios.post(`${API_BASE}/chat`, {
        sessionId,
        message,
        language: 'zh',
      });
      
      setMessages(prev => [...prev, 
        { role: 'user', content: message },
        { role: 'assistant', content: data.messageCN, ...data }
      ]);
      
      return data;
    } finally {
      setLoading(false);
    }
  }, [sessionId, createSession]);

  return { sessionId, messages, loading, sendMessage, createSession };
}
```

### 使用示例

```tsx
function PlanningChat() {
  const { messages, loading, sendMessage } = usePlanningAssistant();
  const [input, setInput] = useState('');

  const handleSend = async () => {
    if (!input.trim()) return;
    await sendMessage(input);
    setInput('');
  };

  return (
    <div>
      <div className="messages">
        {messages.map((msg, i) => (
          <div key={i} className={`message ${msg.role}`}>
            {msg.content}
            {msg.planCandidates && (
              <PlanCandidateList candidates={msg.planCandidates} />
            )}
          </div>
        ))}
      </div>
      <input 
        value={input} 
        onChange={e => setInput(e.target.value)}
        onKeyPress={e => e.key === 'Enter' && handleSend()}
        disabled={loading}
      />
      <button onClick={handleSend} disabled={loading}>
        {loading ? '思考中...' : '发送'}
      </button>
    </div>
  );
}
```

---

## 六、P1 功能增强

### 6.1 人格语言风格优化

三人格（Abu、Dr.Dre、Neptune）现在会生成更有个性、更有温度的发言。

#### 人格特点

| 人格 | 角色 | 风格 | 口头禅示例 |
|------|------|------|-----------|
| 🐻‍❄️ Abu | 安全守护者 | 稳重、可靠、直接 | "这条路，我确认过了" |
| 🐕 Dr.Dre | 节奏管家 | 温暖、关心、贴心 | "别太累，慢慢来" |
| 🦦 Neptune | 方案大师 | 灵活、创意、乐观 | "我有更棒的替代方案！" |

#### 在响应中的体现

```json
{
  "messageCN": "...\n\n🐻‍❄️ **Abu 说**: 我仔细检查过冰岛的路线了——安全可行。放心出发吧！\n🐕 **Dr.Dre 说**: 这个节奏刚刚好——不急不慢。你会享受每一刻的！\n🦦 **Neptune 说**: 冰岛会很棒！别担心，我随时准备着 Plan B。"
}
```

### 6.2 推荐算法优化

目的地推荐现在采用多因素评分算法：

| 评分维度 | 分值 | 说明 |
|----------|------|------|
| 预算匹配度 | 0-25 | 用户预算与目的地消费水平匹配 |
| 季节适合度 | 0-20 | 出行时间与目的地最佳季节匹配 |
| 偏好匹配度 | 0-25 | 用户偏好标签与目的地特点匹配 |
| 人数适合度 | 0-15 | 出行人数与目的地适合人群匹配 |
| 热门程度 | 0-15 | 目的地综合热门度 |

#### 响应示例

```json
{
  "recommendations": [
    {
      "id": "iceland",
      "name": "Iceland",
      "nameCN": "冰岛",
      "matchScore": 87,
      "matchReasons": ["Within your budget", "Perfect season to visit", "Matches your interests"],
      "matchReasonsCN": ["预算友好", "最佳旅行季节", "符合你的兴趣"]
    }
  ]
}
```

### 6.3 用户偏好学习

系统会被动学习用户的旅行偏好，用于优化后续推荐。

#### 学习触发时机

| 行为 | 学习权重 | 说明 |
|------|----------|------|
| destination_selected | 0.3 | 用户选择目的地 |
| plan_generated | 0.2 | 用户请求生成方案 |
| plan_confirmed | 0.6 | 用户确认方案 |
| trip_completed | 1.0 | 用户完成旅行 |
| preference_stated | 0.8 | 用户明确表述偏好 |

#### 学习维度

- **目的地类型偏好**: 自然/城市/海滩/文化/冒险
- **预算范围**: 平均预算水平
- **旅行节奏偏好**: 悠闲/适中/紧凑
- **出行人数习惯**: 独自/情侣/家庭/朋友
- **季节偏好**: 偏好的出行月份
- **活动偏好**: 喜欢的活动类型

#### API 接口

- `GET /agent/planning-assistant/users/:userId/preferences` - 获取学习到的偏好
- `POST /agent/planning-assistant/users/:userId/preferences/clear` - 清除偏好

---

## 七、Swagger 文档

启动服务后访问：

```
http://localhost:3000/api/docs
```

可查看完整的 Swagger API 文档，包括：
- 所有接口的详细参数说明
- 在线测试功能
- 响应示例
