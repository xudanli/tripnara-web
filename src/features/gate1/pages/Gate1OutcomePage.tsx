...
/** 独立 Outcome 复盘页 — 也可从项目详情 Tab 进入 */
export default function Gate1OutcomePage() {
    ...
    <div className="flex min-h-full flex-col bg-neutral-50 dark:bg-neutral-950">
      <div className="border-b bg-background/80 backdrop-blur-sm">
        <DashboardSubpageHeader
          backTo={`/dashboard/gate1/projects/${projectId}`}
          title="Outcome 复盘"
      ...
      <div className="mx-auto w-full max-w-4xl flex-1 space-y-4 px-4 py-6 md:px-6 md:py-8">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" size="sm" asChild>
            <Link to={`/dashboard/gate1/projects/${projectId}`}>返回项目详情</Link>
          </Button>
        </div>

        <Gate1OutcomePanel projectId={projectId} />
      </div>
    </div>
  );
}
