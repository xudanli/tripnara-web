...
## 五附、团队与需求 vs 团队画像（边界说明）

> **P0 产品原则**：两套问卷、两个 Tab，避免用户以为「填了两遍同一张表」。

| 维度 | **团队与需求** Tab | **团队画像** Tab |
|------|-------------------|-----------------|
| **用户任务** | 收集可执行行前需求；催填、看缺口与需求差异 | 理解协作决策人格；看合拍度、摩擦、分摊 |
| **数据来源** | `GET /trips/:id/member-onboarding-profiles` | `decision-profiling/*`（Travel Style、Money DNA、friction-radar） |
| **问卷内容** | 核心愿望、硬性限制、节奏、住宿、餐饮、预算倾向 | 决策风格测评 + 消费 DNA（非重复 onboarding 字段） |
| **成员展示** | **填写进度条** + 缺失维度（不做风格卡） | **旅行风格墙**（styleLabel、关键词、协作提示） |
| **团队级洞察** | 需求差异雷达、需求冲突清单、需求汇总 | 摩擦分、摩擦矩阵、分摊共识、推荐行动 |
| **主操作** | 提醒成员填写行前问卷 | 开始/重新测评、沿用上次结果 |
| **交叉导航** | →「前往团队画像（决策测评）」 | →「前往团队与需求（行前问卷）」 |

**实现**：`COLLAB_TAB_SCOPE_COPY` · `CollabTabScopeHint` · 风格墙 `description` 注明数据来源。
