...
| 层级 | 位置 | Tab 项 | 功能定位 |
|------|------|--------|----------|
| **第一层** | 行程主视图 | 总览 / **规划** / 执行 / 复盘 / 预算 / **团队** | 行程维度的大模块切分 |
| **第二层** | 规划 Tab 内 | 安全 / 节奏 / 修复 / **智能优化** | 规划阶段的不同视角与能力 |
| **第三层** | 智能优化内 | 评估 / 协商（三守护者）/ 实时 | 计划评估与 AI 优化工具 |
...
#### US-P11：团队与协商
> 作为**团队规划者**，我希望能找到团队管理、协商、权重设置等与多人协作相关的能力，并且流程清晰：创建团队 → 设置权重 → 协商 → 查看结论。

**验收标准**：团队能力主入口为**第一层「团队」Tab**；规划 Tab 智能优化内可提供「前往团队 Tab」引导链接（P2）。
...
| US-P11 | 团队规划者 | P1 | **第一层 团队 Tab**（主入口）|
...
| 问题 | 表现 | 影响 |
|------|------|------|
| **认知负担高** | 三层 Tab 同时可见，概念多（安全/节奏/修复/智能优化 + 评估/协商/实时） | 用户不知从何入手，易迷失 |
...
**命名调整**：  
- 原「优化」→「智能优化」或「计划优化」，与第三层「评估/协商（三守护者）/实时」区分。  
...
**现状**：评估 | 协商（三守护者）| 实时，外加 一键优化、评估、权重、协商 等按钮。
...
2. **操作流线性化**：  
   - 评估 → 优化 → 协商（三守护者）→ 实时（执行中）。团队能力主入口为第一层「团队」Tab。  
...
| **Phase 2（已完成）** | 第二层「优化」改名为「智能优化」；第三层增加流程指引、评估空态优化、展开动效 | US-P13 |
...
| US-P11 | 团队能力主入口为第一层「团队」Tab，可被发现；智能优化内可提供「前往团队 Tab」引导（P2）|
...
| **智能优化** | 评估、三守护者协商、实时等计划优化能力（团队能力见第一层团队 Tab）| OptimizationDashboard |
...
| 设计概念 | 代码/组件 |
|----------|-----------|
| 第一层 Tab | `[id].tsx` TabsList: overview / plan / execute / insights / budget / team |
| 综合视图 | `AutoOverview.tsx` |
| 第二层 3 视角（规划内） | `[id].tsx` planViewTab: abu / dre / neptune |
| 第三层 优化工具 | `OptimizationDashboard.tsx` 评估 / 三守护者协商 / 实时（团队能力见第一层团队 Tab）|
