...
判断路线是否应该存在（Should-Exist Gate）要
...
Color System：中性色为主；**裁决四态（gate）** + **冰岛品牌辅助三色（nara）** 分工明确，严禁情绪化大色块与 Tailwind 彩虹装饰色
...
**四态 vs 品牌辅助色**：gate 四态是**裁决语法**；`nara-lava / nara-glacier / nara-tundra` 是**冰岛场景语法**。REJECT 的 UI 默认仍用 `gate-reject`；仅当语义为「封路/气象窗口关闭/绝对不可执行」时，标题/蒙层可升格为 `nara-lava`。ALLOW 的 UI 默认仍用 `gate-allow`；仅当语义为「实地验证/人格匹配/焦虑消除」时，角标/标签用 `nara-tundra`。
...
Abu：边界、盾、门、警戒 —— 色彩倾向：**熔岩红（绝对阻断）** + gate-reject；数据只读面可用 **冰川蓝**

Dr.Dre：节奏、脉冲、呼吸、时间窗 —— 色彩倾向：gate-confirm（需签收节奏），不用 nara 三色抢戏

Neptune：空间、轨迹、结构、修复 —— 色彩倾向：**gate-suggest（冰川蓝 · 结构替换）**；地图 metric 只读用 **nara-glacier**
...
禁止：用熔岩红（nara-lava）表达「软冲突 / 偏好未满足 / 建议调整」——那是 gate-confirm / gate-suggest 的职责

禁止：用冰川蓝（nara-glacier）做 CTA、裁决 Banner、或任何「可点击行动」的主色——它是只读数据色

禁止：用苔原哑绿（nara-tundra）做大面积背景铺底或「种草式成功庆祝」——仅用于小面积验证/人格/可行标签

禁止：在产品 UI 中使用 **紫色 / violet / purple** 作为装饰色——`gate-suggest` 已改为 **冰川蓝（非紫）**；`SUGGEST_REPLACE` 与分流 A 组语义统一走 **nara-glacier / gate-suggest**

禁止：分流 A/B 组用 **饱和色块 / 彩色底** 区分——只用 **字母、左线、muted 底** 等结构手段

[TripNARA 冰岛品牌辅助色（nara · 与 gate 四态分工）]

**设计判定（硬规则）**：以下三色是 TripNARA 在「冰岛决策旅行」语境下的**品牌辅助色**，与 `globals.css` 中的 **gate 四态**（allow / confirm / suggest / reject）**并存、不互相取代**。

- **gate** = 系统裁决语义（ALLOW / NEED_CONFIRM / SUGGEST_REPLACE / REJECT），贯穿 Banner、Diff、Decision Log、工作台状态条。
- **nara** = 冰岛地貌隐喻 + 用户心理锚点，用于**地图、硬核数据、绝对封禁、实地验证、人格结果**等「场景化」表面。

二者关系（必须内化）：

| 品牌辅助色 | Hex | 心理锚点 | 优先映射 gate | 典型场景 |
|-----------|-----|----------|---------------|----------|
| **熔岩红 nara-lava** | `#C25A3C` | 岩浆 = 立刻停手，必须看 B 计划 | **REJECT 的「绝对硬阻断」子层** | Road.is 封路、极光云层过厚不可观测、地图风险蒙层标题、绝对禁止弹窗标题 |
| **冰川蓝 nara-glacier** | `#88C0D0` | 客观事实，零情绪 | **SUGGEST_REPLACE 视觉层（禁止紫）** | 预估总价、公里数、体感温度；Evidence 时间戳/数值；行程单 metric 网格 |
| **苔原哑绿 nara-tundra** | `#5E7D5B` | 冰岛唯一生机 = 焦虑被消除 | **ALLOW 的「已验证/可安心」子层** | 旅行人格结果标签、「该路段已实地验证」角标、可行度标签 |

**与 gate 四态的共存规则**：

1. **NEED_CONFIRM（琥珀 gate-confirm）** 与 **SUGGEST_REPLACE（冰川蓝 gate-suggest）** 仍只走 gate token；**工作台分流 UI 默认克制中性**，仅在裁决 Banner / 风险等语义位用色。
2. **熔岩红** 仅用于「外部现实已关闭可能性」——比 gate-reject 更「地理化、不可协商」；**仍禁止全屏红底**。
3. **冰川蓝** 只用于**只读数字与单位**，不用作链接色、按钮色；数字排版优先 `font-mono-brand` + `nara-glacier-foreground`。
4. **苔原哑绿** 只用于**小面积**标签/角标/细描边（≤ 组件级）。

**Token 命名（工程交付时必须使用，禁止硬编码 Hex）**：

```css
--nara-lava: #C25A3C;
--nara-glacier: #88C0D0;
--nara-tundra: #5E7D5B;
/* 各配 foreground / border / muted 浅底 */
--gate-suggest: var(--nara-glacier-muted); /* 禁止 hue 280 紫色 */
```

Tailwind / 组件引用：`bg-nara-lava-muted`、`text-nara-glacier-foreground`、`border-nara-tundra-border` 等。

**组件级绑定（落地 checklist）**：

| 组件 / 表面 | 用色 |
|------------|------|
| 地图封路/气象蒙层标题 | `nara-lava-foreground` + 可选 `nara-lava-muted` 条 |
| MetricGrid / 行程硬核数据 | `text-nara-glacier-foreground`，标签用 `muted-foreground` |
| 人格测验结果 Chip | `nara-tundra` 系 |
| 工作台 gate Banner / 分流四态 | **仍用 gate-\*** |
| 分流 A/B 组 | **中性卡 + 左线/浅灰底结构区分** |

**验收标准（品牌辅助色）**：

- 一屏内 nara 三色**最多同时出现 2 种**
- 熔岩红出现时必须**伴随 B 计划 / 替代路线 / Evidence 入口**
- 全站 grep：**无 purple/violet** 装饰色
- 苔原绿标签必须**可追溯到验证来源**，否则退回中性 `muted`
...
2. 查看 `tailwind.config.js` 和 `src/styles/globals.css`，了解现有 token（**gate 四态 + nara 冰岛辅助三色**）
